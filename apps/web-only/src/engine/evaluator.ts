import type { Card, HandCategory, Rank } from './types'
...
function getStraightHighCard(ranks: number[]): number | null {
  ...
  for (let i = 0; i <= unique.length - 5; i++) {
    ...
    if (seq.every(r => unique.includes(r))) return top
  }
  if (
    unique.includes(14) &&
    unique.includes(2) &&
...
function getStraightCards(cards: Card[]): Card[] | null {
  ...
  for (let i = 0; i <= uniqueRanks.length - 5; i++) {
    ...
    if (top === undefined) continue
    const seq = [top, top - 1, top - 2, top - 3, top - 4]
    if (seq.every(r => uniqueRanks.includes(r as Rank))) {
      const result: Card[] = []
      for (const r of seq) {
        const card = sortedDesc.find(c => c.rank === (r as Rank))
  ...
  if ([14, 2, 3, 4, 5].every(r => ranks.includes(r as Rank))) {
    const result: Card[] = []
    for (const r of [5, 4, 3, 2] as Rank[]) {
      const card = cards.find(c => c.rank === r)
      if (card) result.push(card)
    }
    const ace = cards.find(c => c.rank === (14 as Rank))
...
function evaluate5(five: Card[]): HandResult {
  ...
  if (isFlush && isStraight) {
    const straightCards = getStraightCards(five) ?? five
    if (straightHigh === 14) {
      return {
        category: 'royalFlush',
        tiebreakers: [14],
        bestFive: straightCards,
        description: CATEGORY_NAMES['royalFlush'],
      }
    }
    return {
      ...
      bestFive: straightCards,
      description: `${CATEGORY_NAMES['straightFlush']} (${straightHigh}高)`,
    }
  }
  ...
  if (!g0) {
    ...
    return {
      ...
      bestFive: sorted,
      description: `${CATEGORY_NAMES['highCard']} (${sorted[0]?.rank ?? 0}高)`,
    }
  }
  ...
  if (g0[1] === 4) {
    ...
    return {
      ...
      description: `${CATEGORY_NAMES['fourOfAKind']} (${quadRank}s)`,
    }
  }
  ...
  if (g0[1] === 3 && g1 && g1[1] === 2) {
    return {
      ...
      bestFive: five,
      description: `${CATEGORY_NAMES['fullHouse']} (${g0[0]}带${g1[0]})`,
    }
  }
  ...
  if (isFlush) {
    ...
    return {
      ...
      tiebreakers: sorted.map(c => c.rank),
      bestFive: sorted,
      description: CATEGORY_NAMES['flush'],
    }
  }
  ...
  if (isStraight) {
    ...
    return {
      ...
      tiebreakers: [straightHigh],
      bestFive: straightCards,
      description: `${CATEGORY_NAMES['straight']} (${straightHigh}高)`,
    }
  }
  ...
  if (g0[1] === 3) {
    ...
    return {
      ...
      bestFive: five,
      description: `${CATEGORY_NAMES['threeOfAKind']} (${triRank}s)`,
    }
  }
  ...
  if (g0[1] === 2 && g1 && g1[1] === 2) {
    ...
    return {
      ...
      bestFive: five,
      description: `${CATEGORY_NAMES['twoPair']} (${highPair}和${lowPair})`,
    }
  }
  ...
  if (g0[1] === 2) {
    ...
    return {
      ...
      bestFive: five,
      description: `${CATEGORY_NAMES['pair']} (${pairRank}s)`,
    }
  }
...
export function compareHands(a: HandResult, b: HandResult): -1 | 0 | 1 {
  const r = compareHandResults(a, b)
